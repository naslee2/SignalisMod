    ...
    public static void arianeMessage_neuroRepo(){
        if (Global.getSector().getMemoryWithoutUpdate().getBoolean("$officeryeong_neuralrepositorystart")){
           MessageIntel arianeMessage_neuroRepo_a = new MessageIntel("Message from Ariane",Misc.getBasePlayerColor());
            ...
            Global.getSector().getCampaignUI().addMessage(arianeMessage_neuroRepo_a, CommMessageAPI.MessageClickAction.INTEL_TAB);

            //Eusan_Nation_NeuralRepo_MessageIntel arianeMessage_neuroRepo_a = new Eusan_Nation_NeuralRepo_MessageIntel();
            //arianeMessage_neuroRepo_a.addLine(messageIntel_header, Misc.getBasePlayerColor());
